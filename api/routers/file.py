...
async def upload_file(db: DbDependency,user: UserDependency, directory_id: int = Form(...), file_update_time: str = Form(...), file: UploadFile = FastAPIFile(...)):
    upload_file = FileUploadData(directory_id=directory_id)

    print('file_update_time_!!!!!!:',file_update_time)
    ...
    if existing_file:
        ...
        existing_file.update_at = datetime.now(pytz.timezone('Asia/Tokyo'))
        ...
        new_file = File(
            ...
            update_at=datetime.now(pytz.timezone('Asia/Tokyo'))
        )
        db.add(new_file)
        db.commit()
        db.refresh(new_file)
...
async def rename_file(db: DbDependency, user: UserDependency, file_rename: FileRename):
    ...
    file.update_at = datetime.now(pytz.timezone('Asia/Tokyo'))
...
async def delete_file(db: DbDependency, user: UserDependency, file_delete: FileDelete):
    ...
    target_file.update_at = datetime.now(pytz.timezone('Asia/Tokyo'))
